...
def test_grouped_leave_supports_partial_daily_review(db_session):
    ...
    assert [day.status for day in application.days] == [
...
def test_conflict_marker_contains_no_schedule_details(db_session):
    ...
    db_session.add_all([performance, actor])
    db_session.flush()
    batch.theater_id = theater.id
    ...
    db_session.flush()
    application = submit_leave_application(
        db_session, actor.id, theater.id, [date(2026, 8, 3)], None, today=date(2026, 7, 19)
    )
