...
from app.models.entities import Actor, LeaveRequest, Performance, Role, Theater, TheaterSlot, Designation, ScheduleAssignment, ImportDraftItem
...
def patch_theater(theater_id: int, payload: TheaterUpdate, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
    return _write(lambda: update_theater(db, theater_id, payload), db)


@router.delete("/theaters/{theater_id}", status_code=204)
def remove_theater(theater_id: int, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
    _write(lambda: delete_theater(db, theater_id), db); return Response(status_code=204)


@router.post("/theaters/{theater_id}/archive", response_model=TheaterRead)
def archive_theater(theater_id: int, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
...
def restore_theater(theater_id: int, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
...
def get_slots(theater_id: int, include_inactive: bool = False, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
...
def post_slot(theater_id: int, payload: TheaterSlotCreate, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
...
def patch_slot(slot_id: int, payload: TheaterSlotUpdate, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
    return _write(lambda: update_theater_slot(db, slot_id, payload), db)


@router.delete("/theater-slots/{slot_id}", status_code=204)
def remove_slot(slot_id: int, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
    _write(lambda: delete_theater_slot(db, slot_id), db); return Response(status_code=204)


@router.post("/theater-slots/{slot_id}/archive", response_model=TheaterSlotRead)
def archive_slot(slot_id: int, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
...
def restore_slot(slot_id: int, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
    return _write(lambda: set_theater_slot_active(db, slot_id, True), db)


@router.get("/theaters/{theater_id}/weekly-template")
def get_template(theater_id: int, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
    return _write(lambda: get_weekly_template(db, theater_id), db)


@router.put("/theaters/{theater_id}/weekly-template")
def put_template(theater_id: int, payload: WeeklyTemplateUpdate, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
...
def patch_role(role_id: int, payload: RoleUpdate, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
    return _write(lambda: update_role(db, role_id, payload), db)


@router.delete("/roles/{role_id}", status_code=204)
def remove_role(role_id: int, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
    _write(lambda: delete_role(db, role_id), db); return Response(status_code=204)


@router.post("/roles/{role_id}/archive", response_model=RoleRead)
def archive_role(role_id: int, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
    return _write(lambda: set_role_active(db, role_id, False), db)


@router.post("/roles/{role_id}/restore", response_model=RoleRead)
def restore_role(role_id: int, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
...
def update_performance(performance_id: int, payload: PerformanceUpdate, _: dict[str, str] = Depends(require_admin), db: Session = Depends(get_db)):
    performance = db.get(Performance, performance_id)
    if performance is None: raise HTTPException(status_code=404, detail="performance_not_found")
    slot = db.get(TheaterSlot, payload.theater_slot_id or performance.theater_slot_id)
    if slot is None: raise HTTPException(status_code=404, detail="theater_slot_not_found")
    if slot.theater_id != performance.theater_id or not slot.is_active: raise HTTPException(status_code=400, detail="invalid_theater_slot")
    performance.theater_slot_id = slot.id
    if payload.performance_date is not None:
        performance.performance_date = payload.performance_date
    performance.slot_name_snapshot = slot.name; performance.start_time_snapshot = slot.start_time
    try: db.commit(); db.refresh(performance)
    except IntegrityError as exc:
        db.rollback(); raise HTTPException(status_code=409, detail="performance_already_exists") from exc
    return performance
...
def _write(operation, db: Session):
    try:
        ...
        db.rollback(); raise HTTPException(status_code=409, detail="duplicate_configuration") from exc
    except ValueError as exc:
