...
from app.models.entities import (
    Actor, ActorRoleCapability, Designation, ImportDraftItem, LeaveRequest, Performance,
    Role, ScheduleAssignment, Theater, TheaterSlot, TheaterWeeklyTemplateEntry, Wish,
)
from app.models.enums import LeaveStatus
from app.schemas.admin import (
    ActorCreate, ActorUpdate, RoleCreate, RoleUpdate, TheaterCreate, TheaterSlotCreate,
    TheaterSlotUpdate, TheaterUpdate,
)
...
def delete_theater(db: Session, theater_id: int) -> None:
    theater = _get_theater(db, theater_id)
    if theater.slots or db.scalar(select(Role.id).where(Role.theater_id == theater_id).limit(1)) or db.scalar(select(Performance.id).where(Performance.theater_id == theater_id).limit(1)):
        raise ReferenceConflict("theater_referenced")
    db.delete(theater); db.commit()
...
def list_theater_slots(db: Session, theater_id: int, include_inactive: bool = False) -> list[TheaterSlot]:
    ...
    return list(db.scalars(statement.order_by(TheaterSlot.sort_order, TheaterSlot.start_time, TheaterSlot.id)))


def update_theater_slot(db: Session, slot_id: int, payload: TheaterSlotUpdate) -> TheaterSlot:
    slot = _get_slot(db, slot_id)
    for key, value in payload.model_dump().items(): setattr(slot, key, value)
    db.commit(); db.refresh(slot)
    return slot


def set_theater_slot_active(db: Session, slot_id: int, active: bool) -> TheaterSlot:
    slot = _get_slot(db, slot_id); slot.is_active = active
    db.commit(); db.refresh(slot)
    return slot
...
def delete_theater_slot(db: Session, slot_id: int) -> None:
    slot = _get_slot(db, slot_id)
    if db.scalar(select(TheaterWeeklyTemplateEntry.id).where(TheaterWeeklyTemplateEntry.theater_slot_id == slot_id).limit(1)) or db.scalar(select(Performance.id).where(Performance.theater_slot_id == slot_id).limit(1)):
...
def get_weekly_template(db: Session, theater_id: int) -> dict[str, list[int]]:
    ...
    entries = db.scalars(select(TheaterWeeklyTemplateEntry).where(TheaterWeeklyTemplateEntry.theater_id == theater_id).order_by(TheaterWeeklyTemplateEntry.id))
    for entry in entries: result.setdefault(entry.weekday, []).append(entry.theater_slot_id)
    return result


def replace_weekly_template(db: Session, theater_id: int, template: dict[str, list[int]]) -> dict[str, list[int]]:
    ...
    if len(slots) != len(ids): raise LookupError("theater_slot_not_found")
    if any(slot.theater_id != theater_id for slot in slots): raise ValueError("slot_belongs_to_other_theater")
    if any(not slot.is_active for slot in slots): raise ValueError("theater_slot_inactive")
    db.execute(delete(TheaterWeeklyTemplateEntry).where(TheaterWeeklyTemplateEntry.theater_id == theater_id))
    for weekday, slot_ids in template.items():
        for slot_id in slot_ids: db.add(TheaterWeeklyTemplateEntry(theater_id=theater_id, weekday=weekday, theater_slot_id=slot_id))
    db.commit()
...
def list_roles(db: Session, theater_id: int | None = None, include_inactive: bool = False) -> list[Role]:
    statement = select(Role).order_by(Role.id)
    if theater_id is not None: statement = statement.where(Role.theater_id == theater_id)
    if not include_inactive: statement = statement.where(Role.is_active.is_(True))
    return list(db.scalars(statement))


def update_role(db: Session, role_id: int, payload: RoleUpdate) -> Role:
    role = _get_role(db, role_id); role.name = payload.name; role.group_name = payload.group_name
    db.commit(); db.refresh(role); return role


def set_role_active(db: Session, role_id: int, active: bool) -> Role:
    role = _get_role(db, role_id); role.is_active = active
    db.commit(); db.refresh(role); return role
...
def delete_role(db: Session, role_id: int) -> None:
    ...
    if any(db.scalar(select(model.id).where(model.role_id == role_id).limit(1)) for model in checks): raise ReferenceConflict("role_referenced")
    db.delete(role); db.commit()


def _get_theater(db: Session, theater_id: int) -> Theater:
    theater = db.get(Theater, theater_id)
    if theater is None: raise LookupError("theater_not_found")
    return theater


def _get_active_theater(db: Session, theater_id: int) -> Theater:
    theater = _get_theater(db, theater_id)
    if not theater.is_active: raise ValueError("theater_inactive")
    return theater


def _get_slot(db: Session, slot_id: int) -> TheaterSlot:
    slot = db.get(TheaterSlot, slot_id)
    if slot is None: raise LookupError("theater_slot_not_found")
    return slot


def _get_role(db: Session, role_id: int) -> Role:
    role = db.get(Role, role_id)
    if role is None: raise LookupError("role_not_found")
    return role
...
def replace_actor_capabilities(db: Session, actor_id: int, role_ids: list[int]) -> Actor:
    ...
    if any(not role.is_active for role in roles if role is not None): raise ValueError("role_inactive")
