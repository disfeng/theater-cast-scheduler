...
def upgrade() -> None:
    ...
    for table in (
        "import_draft_items", "import_drafts", "schedule_assignments", "designations",
        "wishes", "leave_requests", "weekly_batches",
        "performances", "roles",
    ):
    ...
    if "is_active" not in theater_columns:
        op.add_column("theaters", sa.Column("is_active", sa.Boolean(), nullable=False, server_default=sa.true()))
    ...
    if "theater_weekly_template_entries" not in inspector.get_table_names():
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("theater_id", "weekday", "theater_slot_id", name="uq_weekly_template_entry"),
    )
        op.create_index("ix_weekly_template_theater_id", "theater_weekly_template_entries", ["theater_id"])
        op.create_index("ix_weekly_template_slot_id", "theater_weekly_template_entries", ["theater_slot_id"])
    ...
    if "theater_slot_id" not in performance_columns:
        ...
        with op.batch_alter_table("performances") as batch:
            ...
            batch.create_foreign_key("fk_performances_theater_slot_id", "theater_slots", ["theater_slot_id"], ["id"])
...
def downgrade() -> None:
    with op.batch_alter_table("performances") as batch:
        ...
        batch.create_unique_constraint("uq_performance_theater_date_slot", ["theater_id", "performance_date", "slot"])
    with op.batch_alter_table("roles") as batch:
    ...
    op.add_column("theaters", sa.Column("default_weekly_template", sa.JSON(), nullable=False, server_default="{}"))
