...
import type { CardOperationResult, TopThreeGrantDefaults } from "../../features/entitlements/top-three-grants";
...
const readOnly=computed(()=>batch.value?.status==="granted"),totalItems=computed(()=>rows.reduce((sum,row)=>sum+(Number(row.quantities[props.definition.id])||0),0)),unresolved=computed(()=>rows.filter(row=>!grantRowIsResolved(row)).length);
function hasContent(){return rows.length>0||!!batch.value}
function isValid(){return totalItems.value>0&&!unresolved.value&&!!props.defaults.source_label.trim()}
...
async function confirmGrant():Promise<CardOperationResult>{if(!isValid())return {actorId:props.actor.id,actorName:props.actor.display_name,outcome:"skipped",reason:unresolved.value?"存在待确认玩家":"未填写完整"};pending.value="confirm";try{const saved=await persist();if(!auth.token)throw new Error("未登录");const next=await adminApi.confirmTheaterGrantBatch(auth.token,props.theaterId,saved.id,`grant-${saved.id}-${Date.now()}`);batch.value=next;emit("batch-change",next);return {actorId:props.actor.id,actorName:props.actor.display_name,outcome:"granted"}}catch(e:any){return {actorId:props.actor.id,actorName:props.actor.display_name,outcome:"failed",reason:e.message}}finally{pending.value=""}}
