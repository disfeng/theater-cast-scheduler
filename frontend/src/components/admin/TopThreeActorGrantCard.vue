<template>
  <article class="actor-card" data-testid="top-three-actor-card">
    <header>
      <div><span class="eyebrow">绑定演员道具</span><h3>{{ definition.display_name }} · {{ actor.display_name }}</h3></div>
    ...
    <el-table :data="rows" border size="small" empty-text="尚未添加榜单玩家">
      ...
      <el-table-column :label="definition.display_name" width="180" align="center"><template #default="{row}"><el-input-number v-model="row.quantities[definition.id]" :min="0" :max="100" :disabled="readOnly" controls-position="right"/></template></el-table-column>
...
function payload():GrantBatchPayload{const label=`${props.defaults.source_label.trim()} · ${props.actor.display_name}`;const month=props.defaults.source_month?`${props.defaults.source_month}-01`:null;return {grant_mode:"by_actor",source_type:props.defaults.source_type,source_month:month,source_label:label,title:label,grant_date:props.defaults.grant_date||null,default_expires_at:toIsoEndOfDay(props.defaults.default_expiry),notes:null,bound_actor_id:props.actor.id,items:expandGrantItems(rows,month,label,toIsoEndOfDay(props.defaults.default_expiry),props.actor.id)}}
