<template>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="saving" :disabled="!displayName.trim()" @click="save">保存演员</el-button>
    </template>
  </el-drawer>
</template>
...
async function save() {
  ...
  try {
    ...
    if (!actorId) {
      const created = await adminApi.createActor(props.token, { display_name: displayName.value.trim(), ...basePayload });
      actorId = created.id;
      createdActorId.value = created.id;
    } else if (props.actor) {
      await adminApi.updateActor(props.token, actorId, basePayload);
    }
