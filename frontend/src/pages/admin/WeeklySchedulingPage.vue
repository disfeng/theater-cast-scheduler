  ...
  <section class="scheduling-page">
    ...
    <el-card v-else shadow="never" class="matrix-card">
      <div class="matrix-scroll">
        <table class="schedule-matrix schedule-matrix--compact">
          <thead>
            <tr>
            ...
            <tr v-for="performance in activeWeek.workspace.performances" :key="performance.id">
              ...
              >
                <el-select
                  ...
                  :model-value="actorIdAt(performance.id, role.id) ?? ''"
                  :disabled="isLocked(performance.id, role.id)"
                  size="small"
                  filterable
                  :teleported="false"
                  placeholder="待安排"
                  @change="setActor(performance.performance_date, performance.id, role.id, Number($event) || null)"
                >
                  <el-option label="待安排" value="" />
                  <el-option
                    v-for="actor in actorsFor(performance.performance_date, role.id)"
                    :key="actor.id"
                    :label="`${actor.display_name} · 本周${actorWeeklyCount(performance.performance_date, actor.id)}`"
                    :value="actor.id"
                  />
                </el-select>
...
const statusText = computed(() => ({ uncreated: "未保存", draft: "草稿", ready: "草稿", partial: "部分发布", scheduled: "已发布" }[activeWeek.value?.workspace.status || "uncreated"]));
...
function dayHasChanges(performanceDate: string) {
  return performancesForDate(performanceDate).some((row) => row.has_unpublished_changes);
}
function dayIsPublished(performanceDate: string) {
  const rows = performancesForDate(performanceDate);
  return Boolean(rows.length) && rows.every((row) => row.publication_status === "published");
}

async function publishDay(performanceDate: string, confirm_republish = false): Promise<void> {
  const weekStart = weekStartForDate(performanceDate);
  if (monthState.value[weekStart]?.dirty && !await saveAllDrafts()) return;
  const week = monthState.value[weekStart];
  if (!week || !theaterId.value) return;
  publishingDay.value = performanceDate;
  try {
    const result = await adminApi.publishScheduleDay(token(), {
    ...
    monthState.value = {
      ...monthState.value,
      [weekStart]: createMonthState([result.workspace])[weekStart],
    };
    ElMessage.success(`${formatDate(performanceDate)}排班已发布`);
  } catch (err: any) {
    const detail = err instanceof ApiError && err.detail && typeof err.detail === "object"
      ? err.detail as { code?: string; added?: number; changed?: number; removed?: number }
      : null;
    if (detail?.code === "republish_confirmation_required") {
      try {
        await confirmAction({
          title: `重新发布${formatDate(performanceDate)}排班`,
          message: `本次将新增 ${detail.added || 0} 项、替换 ${detail.changed || 0} 项、移除 ${detail.removed || 0} 项演员安排。演员端与待发通知将同步更新。`,
          tone: "warning",
          confirmButtonText: "确认重新发布",
        });
        await publishDay(performanceDate, true);
      } catch { /* 管理员取消重新发布 */ }
    } else if (isIncompletePerformanceError(err)) {
      await confirmAction({ title: "当日排班不完整", message: "当天存在已开始安排但尚未补齐全部角色的场次。", tone: "danger", alert: true, confirmButtonText: "返回补充" });
    } else {
      ElMessage.error(err.message || "单日发布失败");
    }
  } finally {
    publishingDay.value = "";
  }
