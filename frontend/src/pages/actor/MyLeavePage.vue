...
async function load(){if(!auth.token)return;loading.value=true;try{applications.value=await actorApi.getLeaveApplications(auth.token)}catch{applications.value=[]}finally{loading.value=false}}
